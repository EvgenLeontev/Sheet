...
const int TOTAL_ENGLISH_LETTERS_IN_ALPHABET = 26;
...
std::string Position::ToString() const {
    ...
    int column = col;
    while (column >= 0) {
        result.insert(result.begin(), 'A' + column % TOTAL_ENGLISH_LETTERS_IN_ALPHABET);
	    column = column / TOTAL_ENGLISH_LETTERS_IN_ALPHABET - 1;
    }
...
Position Position::FromString(std::string_view str) {
    ...
    int col = 0;
    for (char ch : letters) {
        col *= TOTAL_ENGLISH_LETTERS_IN_ALPHABET;
        col += ch - 'A' + 1;
    }
